# Network Insurance Launch Checklist

Do not treat the website as final until these placeholders and reviews are complete.

## Business Information

- [ ] Public business email
- [ ] Public business phone number
- [ ] Advisor's approved professional title
- [ ] Approved professional headshot
- [ ] Public license information, if approved for display
- [ ] Approved states served
- [ ] Approved service-area wording
- [ ] Approved business address, if applicable
- [ ] Approved social links
- [ ] Approved carrier logos and permission to use them
- [ ] Approved coverage categories and products offered
- [ ] Approved wording for life, health, annuities, supplemental, dental, vision, accident, critical illness, hospital indemnity, and disability income protection

## Lead Capture

- [ ] Formspree endpoint
- [ ] Thank-you URL
- [ ] Lead notification email
- [ ] Google Sheets automation
- [ ] CRM webhook
- [ ] Test lead proof
- [ ] Spam/honeypot behavior verified
- [ ] Health insurance conditional fields reviewed
- [ ] Life insurance conditional fields reviewed
- [ ] Annuity conditional fields reviewed
- [ ] Sensitive-information warning reviewed

## Scheduling

- [ ] Google Calendar appointment URL confirmed
- [ ] Scheduler fallback contact wording approved
- [ ] Appointment-click tracking approved, if used

## Cross-Linking

- [ ] Main Advisor insurance website URL
- [ ] Main broker website links back to Network Insurance as the insurance education and coverage solutions website

## Legal and Compliance

- [ ] Final consent language
- [ ] Final insurance disclaimer
- [ ] Final privacy policy
- [ ] Final terms
- [ ] Fee language for coverage reviews
- [ ] Life insurance underwriting, policy loan, surrender charge, lapse, and tax language reviewed
- [ ] Indexed universal life illustration and cash-value caveats reviewed
- [ ] Annuity surrender charge, liquidity, income rider, tax, carrier-strength, and suitability language reviewed
- [ ] Supplemental limited-benefit and major-medical-not-replacement language reviewed
- [ ] Health insurance eligibility, provider network, formulary, and subsidy language reviewed
- [ ] Professional legal/compliance review completed

## SEO and Tracking

- [ ] Social-sharing image
- [ ] Google Analytics ID, if approved
- [ ] Google Search Console verification
- [ ] Meta Pixel ID, if approved
- [ ] UTM/referral tracking plan
- [ ] Analytics/cookie notices, if required
- [ ] Final custom domain, if any

## Content

- [ ] Real video URLs
- [ ] Video transcripts or summaries
- [ ] Future article sources
- [ ] Date-reviewed process for educational content
- [ ] Health insurance articles reviewed
- [ ] Life insurance articles reviewed
- [ ] Annuity articles reviewed
- [ ] Supplemental insurance articles reviewed

## Final QA

- [ ] Home page
- [ ] Life Insurance page
- [ ] Health Insurance page
- [ ] Annuities page
- [ ] Supplemental Coverage page
- [ ] Mobile navigation
- [ ] Desktop navigation
- [ ] Lead form required fields
- [ ] Consent checkbox
- [ ] Error states
- [ ] Success state
- [ ] Thank-you page
- [ ] Appointment link
- [ ] Internal links
- [ ] External links
- [ ] Images
- [ ] Mobile responsiveness
- [ ] Keyboard navigation
- [ ] Focus states
- [ ] Color contrast
- [ ] Browser console
- [ ] Missing files
- [ ] GitHub Pages subdirectory paths
- [ ] Metadata
- [ ] Sitemap
- [ ] Robots file
- [ ] Structured data
- [ ] 404 page
- [ ] Page-load performance
